...
class EventLog:
    ...
    def preprocess_log(self):
        ...
        all_columns = self.log.columns.tolist()
        selected_columns = []
        print("Choose attributes from the event log to be included in the knowledge graph:")
    ...
    def create_edges(self):
        with open(f"{self.name}/edges.tsv", "w", newline='') as file:
    ...
    def create_nodes(self):
        ...
        with open(f"{self.name}/nodes.json", "w") as json_file:
    ...
    def create_knowledge_graph(self):
        ...
        with open(f"{self.name}/nodes.json", "r") as file:
            nodes = json.load(file)
        edges = pd.read_csv(f"{self.name}/edges.tsv", sep='\t')
