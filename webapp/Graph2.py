...
class Graph:
    ...
    def visualize_graph(self, save_name, index, graph):
        g = nx.DiGraph()
        ...
        nx.draw_networkx_labels(g, pos, labels=node_labels, font_size=10, font_color='black', font_weight='bold', ax=ax)

        edge_labels = {(src, dst): f'Freq: {data["Frequency"]}\nAvg Time: {data["Average_time"]}' for src, dst, data in
                       g.edges(data=True)}
        ...
        # save graph
        self.save_graph(graph, f"{save_name}/graphs/{index}.pkl")
    ...
    def embed_graph(self, save_name, embedder):
        ...
        if len(embedded_node_attrs) == 1:
            x = embedded_node_attrs[0]
        else:
    ...
    def retrieve_subgraph_pcst(self, question, embedder, topk, topk_e, cost_e):
