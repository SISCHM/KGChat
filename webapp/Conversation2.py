...
class Conversation:
    ...
    def ask_question(self, graph, question):
        ...
        subgraph = self.know_g.retrieve_subgraph_pcst(question, TextEmbedder.TextEmbedder(),3,3,0.5)
        subgraph_file = f"chats/{self.know_g.name}/graphs/{len(self.prev_conv) + 1}.pkl"
        self.graph_util.save_graph(subgraph, subgraph_file)# save Graph 
        self.prev_conv[len(self.prev_conv) + 1] = {'question': question, 'text_g': text_g, 'answer': answer, 'subgraph_file': subgraph_file}
...
if __name__ == '__main__':
    ...
    input_question = "What activity goes after Payment?"
    subgraph = know_g.retrieve_subgraph_pcst(input_question, embedder,3,3,0.5)
    sub_graph=subgraph
    subgraph.visualize_graph(event_log.name,1,sub_graph)
